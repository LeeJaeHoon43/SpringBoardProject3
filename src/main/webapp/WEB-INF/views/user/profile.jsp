													...
													<tbody>
														<c:forEach var="articleVO" varStatus="i"
															...
															<tr>
																<td>${i.index + 1}</td>
																<td><a
																	href="${path}/article/paging/search/read?articleNo=${articleVO.articleNo}">
																		<c:choose>
																			<c:when test="${fn:length(articleVO.title) > 30}">
																				<c:out
																					value="${fn:substring(articleVO.title, 0, 29)}" />.... </c:when>
																			<c:otherwise>
													...
													<tbody>
														<c:forEach var="userReply" varStatus="i"
															...
															<tr>
																<td>${i.index + 1}</td>
																<td><a
																	href="${path}/article/paging/search/read?article_no=${userReply.articleVO.articleNo}">
																		<c:choose>
																			<c:when
																				test="${fn:length(userReply.articleVO.title) > 10}">
																				<c:out
																					value="${fn:substring(userReply.articleVO.title, 0, 9)}" />.... </c:when>
																			<c:otherwise>
																...
																<td><c:choose>
																		<c:when test="${fn:length(userReply.replyText) > 10}">
																			<c:out
																				value="${fn:substring(userReply.replyText, 0, 9)}" />.... </c:when>
																		<c:otherwise>
																			<c:out value="${userReply.replyText}" />
																		</c:otherwise>
																	</c:choose></td>
																<td><fmt:formatDate pattern="yyyy-MM-dd"
																		value="${userReply.regDate}" /></td>
															</tr>
														</c:forEach>
													</tbody>
												</table>
